...
class TasksDockerTestCase(TestCase):
    ...
    @patch('release_dashboard.tasks.docker_fetch_info')
    @patch('release_dashboard.utils.docker.get_docker_info',
           side_effect=fake_tag)
    def test_docker_fetch_all(self, dgi, dfi):
        result = tasks.docker_fetch_all.delay()
        self.assertTrue(result.successful())
        dgi.assert_called_once_with("_catalog")
        dfi.assert_called_once_with('data-hal')
